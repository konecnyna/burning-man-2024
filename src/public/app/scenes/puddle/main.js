...
socket.on('hand_detect_new', (data) => {
  try {
    ...
    // Map the positions to normalized values used in your shaders
    mouseX = map(posX, window.innerWidth, -1.0, 1.0);
    mouseY = map(posY, window.innerHeight, -1.0, 1.0);

    // Update uniforms for all the materials that use mouseX and mouseY
    materialFB2.uniforms.mouseX.value = mouseX;
    material1.uniforms.mouseX.value = mouseX;
    materialBump.uniforms.mouseX.value = mouseX;
    materialFB2.uniforms.mouseY.value = mouseY;
