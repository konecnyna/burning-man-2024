...
function handleHandTracking(hands) {
  ...
  hands.forEach((hand, index) => {
    const palmCenter = hand.palm_center;
    if (!palmCenter) return;
    
    // Convert normalized coordinates to pixel coordinates
    let posX = scaleByPixelRatio(palmCenter.x * canvas.clientWidth);
    let posY = scaleByPixelRatio(palmCenter.y * canvas.clientHeight);
...
socket.on('event', (data) => {
  try {
    ...
    if (data.type === 'hand_lost') {
      // Gradually fade out hand pointers instead of immediate removal
      pointers.forEach(p => {
        if (p.id !== -1) { // Not mouse pointer
...
function pointerPrototype() {
  ...
  this.moved = false;
  this.color = [30, 0, 300];
  this.lastSeen = Date.now(); // Track when this pointer was last updated
}
...
function update() {
  ...
  if (resizeCanvas())
    initFramebuffers();
  updateColors(dt);
  cleanupStalePointers(); // Clean up inactive hand pointers
  applyInputs();
  if (!config.PAUSED)
    step(dt);
  render(null);
...
function cleanupStalePointers() {
  ...
  // Remove hand pointers that haven't been updated recently
  pointers = pointers.filter(p => {
    if (p.id === -1) return true; // Keep mouse pointer
